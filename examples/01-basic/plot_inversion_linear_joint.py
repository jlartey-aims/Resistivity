...
def run(N=100, plotIt=True):
    ...
    prob1 = Problem.LinearProblem(mesh, F=G, modelMap=wires.m1)
    survey1 = Survey.LinearSurvey()
    survey1.pair(prob1)
    survey1.makeSyntheticData(m, std=0.01)

    prob2 = Problem.LinearProblem(mesh, F=G, modelMap=wires.m2)
    ...
    if plotIt:
        ...
        for i in range(prob1.F.shape[0]):
            axes[0].plot(prob1.F[i, :])

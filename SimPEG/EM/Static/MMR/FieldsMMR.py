...
class FieldsMMR(SimPEG.Problem.Fields):
    knownFields = {}
    dtype = float
...
class Fields_CC(FieldsMMR):
    knownFields = {'ASolution': 'CC'}
    aliasFields = {
                    'A': ['ASolution', 'F', '_a'],
                    'H' : ['ASolution', 'E', '_h'],
                    'B' : ['ASolution', 'F', '_b'],
    ...
    def __init__(self, mesh, survey, **kwargs):
        FieldsMMR.__init__(self, mesh, survey, **kwargs)
    ...
    def _GLoc(self, fieldType):
        if fieldType == 'h':
            return 'E'
        elif fieldType == 'a' or fieldType == 'b':
            return 'F'
        else:
            raise Exception('Field type must be phi, e, j')

    def _a(self, phiSolution, srcList):
        return phiSolution
    ...
    def _h(self, phiSolution, srcList):
        """
            .. math::
                \mathbf{j} = \mathbf{M}^{f \ -1}_{\rho} \mathbf{G} \phi
        """
        return self.MeMuI*self.prob.mesh.edgeCurl.T*ASolution

    def _b(self, phiSolution, srcList):
     #   """
     #       In HJ formulation B is not well-defined!!
     #       .. math::
     #           \vec{e} = -\nabla \phi
     #   """
